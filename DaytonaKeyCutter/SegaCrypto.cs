...
{
    public class SegaCrypto
    {
        private readonly UInt32[] P = new UInt32[18];
        private readonly UInt32[,] S = new UInt32[4, 256];
        ...
        //8C06C6AC
        public SegaCrypto(byte[] key)
        {
            UInt32 shifted = 0;
            int keyIndex = 0;

            // Copy starting values
            Buffer.BlockCopy(P_TABLE, 0, P, 0, P_TABLE.Length);
            Buffer.BlockCopy(S_TABLE, 0, S, 0, S_TABLE.Length);
        ...
        public void Encrypt(byte[] data, int length)
        {
            ...
            {
                uint xl = (uint)((data[i + 3]) | (data[i + 2] << 8) | (data[i + 1] << 16) | (data[i + 0] << 24));
                uint xr = (uint)((data[i + 7]) | (data[i + 6] << 8) | (data[i + 5] << 16) | (data[i + 4] << 24));
        ...
        private void EncipherBlock(ref UInt32 xl, ref UInt32 xr)
        {
            UInt32 temp;
        ...
        private void DecipherBlock(ref UInt32 xl, ref UInt32 xr)
        {
            UInt32 temp;
        ...
        private static void SwapEndian(ref uint input)
        {
            input = ((input >> 24) & 0xff) |
                    ((input << 8) & 0xff0000) |
                    ((input >> 8) & 0xff00) |
